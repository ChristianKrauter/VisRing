...
{
  if (global_grayscale > 14) {
    global_grayscale = 0;
  }
  global_grayscale += 2;
  Serial.println(global_grayscale);
}
...
void shapeExample()
{
         ...
         (ball_X + ball_rad < flexibleOLED.getDisplayWidth() - 2))
  {
    ...
    // Draw the Pong Field
    flexibleOLED.clearDisplayGS(CLEAR_BUFFER); //Save time. Only clear the local buffer.
    // Draw an outline of the screen:
    flexibleOLED.rectGS(0, 0, flexibleOLED.getDisplayWidth() - 1, flexibleOLED.getDisplayHeight(), global_grayscale);
    // Draw the center line
    flexibleOLED.rectFillGS(flexibleOLED.getDisplayWidth() / 2 - 1, 0, 2, flexibleOLED.getDisplayHeight(), global_grayscale);
    // Draw the Paddles:
    //flexibleOLED.rectFill(paddle0_X, paddle0_Y, paddleW, paddleH);
    //flexibleOLED.rectFill(paddle1_X, paddle1_Y, paddleW, paddleH);
    flexibleOLED.rectFillGS(paddle0_X, paddle0_Y, paddleW, paddleH, global_grayscale);
    flexibleOLED.rectFillGS(paddle1_X, paddle1_Y, paddleW, paddleH, global_grayscale);
    // Draw the ball:
    flexibleOLED.circleGS(ball_X, ball_Y, ball_rad, global_grayscale);
...
void printTitle(String title, int font)
{
  ...
  flexibleOLED.clearDisplayGS(); //Clear everything
}
