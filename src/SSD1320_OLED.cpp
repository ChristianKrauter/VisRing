  ...
  for (uint8_t rows = 0; rows < 32; rows++)
  {
    for (uint8_t columns = 0; columns < 20; columns++)
    {
      ...
      for (uint8_t bitNumber = 8; bitNumber > 0; bitNumber -= 2)
      {
        ...
        if ((originalByte & (1 << (bitNumber - 1))) != 0)
          newByte |= 0x0F;
        if ((originalByte & (1 << (bitNumber - 2))) != 0)
          newByte |= 0xF0;
...
void SSD1320::clearDisplay(uint8_t mode)
{
  if (mode == CLEAR_DISPLAY || mode == CLEAR_ALL) // Clear the RAM on the display
  {
    ...
    if (mode == CLEAR_ALL)
      memset(screenMemory, 0, (_displayHeight * _displayWidth / 8)); // Clear the local buffer as well
  }
...
void SSD1320::clearDisplayGS(uint8_t mode)
{
  if (mode == CLEAR_DISPLAY || mode == CLEAR_ALL) // Clear the RAM on the display
  {
    ...
    if (mode == CLEAR_ALL)
      memset(screenMemoryGS, 0, (_displayHeight * _displayWidth / 2)); // Clear the local buffer as well
  }
  else // Clear the local buffer
  {
