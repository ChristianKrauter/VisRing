...
void get_tmp_data()
{
  ...
  ALL_MAX[total_num_data - 2] = temp.temperature;
}

void display_tmp(void)
{
  long diff_display = millis() - display_time;
  //  Serial.println(diff_display);
  //  delay(5000);
...
void get_imu_data(void)
{
  ...
  if (imu.dataReady())
  {
    ...
    float ALL_IMU[] = {
        imu.accX() * 0.01f, imu.accY() * 0.01f, imu.accZ() * 0.01f, // acceleration is measured in m/s^2
        imu.gyrX(), imu.gyrY(), imu.gyrZ(),                         // magnetometer is measured in uT
        imu.magX(), imu.magY(), imu.magZ(),                         // gyro is measured in radians/s
    };
